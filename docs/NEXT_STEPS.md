...
| --- | --- | --- | --- |
| 执行者 A：核心开发 | [任务书 A](TASK_CORE.md) | A0、N1–N4：存储端口、迁移、数据制作与隔离、配对证据、实验脚本、真实实验与报告初稿 | A0，然后 N1 → N2 → N3 → N4 |
| 执行者 B：展示开发 | [任务书 B](TASK_PRESENTATION.md) + [PPT 设计](PRESENTATION_PLAN.md) | B0–B4：展示契约、只读 API、导出、前端、截图、PPT 与讲稿初稿 | B0，并行准备 B1 与 B4 |
...
本轮规划起点已有 CLI、本地 FastAPI 执行入口及 SQLite 证据存储；前端和任务书列出的 GET 查询端点尚待实现。已有历史真实运行包含 Gate Reject，尚不能宣称已证实充分样本的真实晋级收益。任务书中的拟议文件、接口和复选框不代表已完成实现。
...
Tool Policy 候选、贡献分析与消融、质量/效率规则扩展、自动 Stable/Reopen/Rollback、claim 恢复、三类任务的完整对照与 Final Test，以及更完整的可视化仍按 ROADMAP 推进。本轮不提前派发这些任务。下一轮由负责人根据验收结果划定范围，继续由执行者准备代码、数据和实验材料，负责人审查与确认。
